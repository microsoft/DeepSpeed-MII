...
class MIIClient:
    ...
    async def _request_async_response_stream(self, request_dict, **query_kwargs):
        ...
        async for response in getattr(self.stub, task_methods.method_stream_out)(proto_request):
            yield task_methods.unpack_response_from_proto(response)

    def generate(self, prompt: str, streaming_fn: Callable = None, **query_kwargs: Dict[str, Any]):
        if not isinstance(prompt, str):
            raise RuntimeError("MII client only supports a single query string, multi-string will be added soon")
    ...
    def _generate_stream(self, callback, request_dict: Dict[str, str], **query_kwargs: Dict[str, Any]):

        async def put_result():
            response_stream = self._request_async_response_stream(request_dict, **query_kwargs)
    ...
    def terminate_server(self):
        ...
        if self.mii_config.enable_restful_api:
            requests.get(f"http://localhost:{self.mii_config.restful_api_port}/terminate")
