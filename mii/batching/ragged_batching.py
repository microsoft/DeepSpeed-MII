...
class MIIPipeline(RaggedBatchBase):
    ...
    def call_dbg(self,
        ...
        if isinstance(prompts, str):
            prompts = [prompts]
        outputs: List[Response] = []
        uids_running: List[int] = []
        uids_complete_order: List[int] = []

        requests = []
        for uid, input in enumerate(prompts):
            request_kwargs = generate_kwargs.copy()
            requests.append((uid, input, request_kwargs))

        req = requests.pop(0)
        self._put_request(req[0], req[1], req[2])
        uids_running.append(req[0])
        ...
        if self.is_rank_0:
            ...
            while uids_running \
                ...
                print(f"iteration {iteration} running_requests {uids_running} scheduled_requests {self.scheduled_requests.requests_to_run.uids} flush_requests {self.scheduled_requests.requests_to_flush.uids}")

                if iteration % delay == 0 and len(requests) > 0:
                    req = requests.pop(0)
                    self._put_request(req[0], req[1], req[2])
                    uids_running.append(req[0])
                    print(f"Request uid={req[0]} added to queue")
