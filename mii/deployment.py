...
def deploy(model_name,
    ...
    if deployment_type == DeploymentType.LOCAL:
        return _deploy_local(model_name, local_model_path = local_model_path, parallelism_config = parallelism_config)

    assert aml_workspace is not None, "Workspace cannot be none for AML deployments"   
    
    if deployment_type == DeploymentType.LOCAL_AML:
        return _deploy_aml_local(model_name, aml_workspace, aml_deployment_name, aml_model_tag = aml_model_tag, parallelism_config=parallelism_config)
    ...
    return _deploy_aml_on_aks(model_name, aml_workspace, aks_target, aks_deploy_config, aml_deployment_name, aml_model_tag=aml_model_tag, parallelism_config=parallelism_config)

#TODO support this properly
#Fetch the specified model if it exisits. If the model does not exist and its the default supported model type, create the model and register it
def _get_aml_model(model_name, aml_model_tag, aml_workspace):
    if model_name in aml_workspace.models.keys():
        return Model(workspace=aml_workspace, name=model_name, id=f"{model_name}:1", version=1, tags={}, properties={})
    else:
...
def _deploy_aml_on_aks(model_name, 
    ...
    inference_config = InferenceConfig(
        ...
        entry_script=os.path.join("models", model_name,"score.py"))
