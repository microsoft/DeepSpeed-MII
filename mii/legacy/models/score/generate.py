...
def create_score_file(mii_config):
    if len(mii.__path__) > 1:
        logger.warning(f"Detected mii path as multiple sources: {mii.__path__}, might cause unknown behavior")

    with open(os.path.join(mii.__path__[0], "models/score/score_template.py"), "r") as fd:
        score_src = fd.read()
    ...
    with open(generated_score_path(mii_config.deployment_name, mii_config.deployment_type), "w") as fd:
...
def generated_score_path(deployment_name, deployment_type):
    if deployment_type == DeploymentType.LOCAL:
        ...
        score_path = os.path.join(mii.aml_related.utils.aml_output_path(deployment_name), "code")
