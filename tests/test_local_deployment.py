...
def mii_config(dtype: str, tensor_parallel:int, port_number: int):
    return {'dtype': dtype, 'tensor_parallel': tensor_parallel, 'port_number': port_number}

@pytest.fixture(scope="function")
def deployment_config(task_name: str, model_name: str, query: dict, mii_config: dict, enable_deepspeed:bool, enable_zero:bool, ds_config:dict):
...
def local_deployment(deployment_config, expected_failure):
    if expected_failure is not None:
        with pytest.raises(expected_failure):
            mii.deploy(
                task_name=deployment_config.task_name,
...
@pytest.mark.local
@pytest.mark.parametrize("mii_config", [{'dtype': 'fp16', 'tensor_parallel':1}, {'dtype': 'fp32', 'tensor_parallel':1}])
